<?php
    ...
    $list = Doctrine_Query::create()->from($module->tableName . ' t')->orderBy('name')->execute();
                        ...
                        <table class="table" data-show-toggle="true">
                        	...
                        	<tbody>
                        		...
                        		<tr id="rowdelForm<?=$row->token?>">
                        			<td><?=($row->isActive==1)?'<i class="material-icons uk-text-success">&#xE8DC;</i>':'<i class="material-icons uk-text-danger">&#xE8DB;</i>'?></td>
                                    <td><?=date('d-m-Y', strtotime($row->createdAt))?></td>
                                    <td><?php if($img){ ?>
	                                    <img class="img_thumb" src="<?=$_config->websitemm.$img->filepath.$img->filename?>" />
	                                    <?php }else{ 
		                                    echo 'Sin imagen';
	                                    } ?>
	                                </td>    
                                    ...
                                    <td>
                                        ...
                                        <form id="delForm<?=$row->token?>">
                                            <input type="hidden" name="form_ajax" id="form_ajax" value="delete<?=$tableName?>">
